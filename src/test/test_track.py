...
def test_get_track_list() -> None:
    track_to_length = {Track.DATA_STRUCTURES: 73, Track.ALGORITHMS: 45}
    ...
    for track, expected_length in track_to_length.items():
        ...
        definitions_in_tracks.update({item for item in track_list})

    all_definition_keys = {definition.key for definition in get_all_definitions()}
    definitions_diff = all_definition_keys - definitions_in_tracks
